...
speedscatter %>% #KEEPING
  drop_na(values) %>%
  ggplot(data = speedscatter, mapping = aes(x = GDP.per.capita, y = values, colour = gini)) +
  geom_point() + geom_smooth(method="lm", col="black")
...
```{r fixingbroken, eval=FALSE, echo=FALSE, warning=FALSE, message=FALSE, fig.height = 6, fig.width = 14}
#remove all non-pg13 commenst when setting echo to true
library(tidyverse)
library(ggplot2)
library(maps)
...
speed_dataset <- read.csv("worldwideInternetSpeed.csv")
speed_dataset$region <- countrycode(speed_dataset$region, "iso2c", "iso3c")
write.csv(speed_dataset,"./worldwideInternetSpeed.csv", row.names=FALSE)
