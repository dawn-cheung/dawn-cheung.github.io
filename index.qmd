...
speedscatter %>% #probelm
  ...
  geom_point()
speedscatter %>%
  select(country, year, gini, GDP.per.capita) #debug line
...
pricescatter %>% 
        drop_na(values) %>%
  ggplot(data = pricescatter, mapping = aes(x = gini, y = values, colour = region_wb)) +
  geom_point() + geom_smooth(method="lm", col="black")
...
AgreeDisagreePie <- function(percent, my_title) {
  ...
  ggplot(data, aes(x="", y=amount, fill=category)) +
    ...
    theme(plot.title = element_text(size=15),
          plot.margin = margin(0, 0.2, 0, 1.7, "cm"))
}
