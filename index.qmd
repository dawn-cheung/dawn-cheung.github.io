...
```{r PRICEoverACCESS, eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE, fig.height = 13, fig.width = 14}
...
for (i in 1:length(distinct_CR)) {
  graphly = graphly %>% 
    add_trace(data = diff_data[[i]], x = ~price, y = ~percent_access, type = 'scatter', name = distinct_CR[[i]], mode = 'markers', alpha = 0.65, marker = list(size = 10.2))
}
