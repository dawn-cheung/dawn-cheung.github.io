...
speedscatter %>% #KEEPING
  ...
  geom_point() + 
  labs(x = "Speed of Internet", y = "GDP per capita", title = str_wrap("How economic development affects mean speed of internet", 40)) +
  geom_smooth(method="lm", col="black")  +
  theme(plot.title=element_text(size=35))
...
```{r fixingbroken, eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE, fig.height = 6, fig.width = 14}
#remove all non-pg13 commenst when setting echo to true
speedscatter %>% #KEEPING
  drop_na(values) %>%
  ggplot(data = speedscatter, mapping = aes(x = values, y = GDP.per.capita, colour = gini)) +
