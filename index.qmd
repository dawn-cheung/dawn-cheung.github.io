...
<iframe height="435" width="145%" frameborder="no" src="https://myshinyappplease.shinyapps.io/MyShinyApp/">
...
```{r unbroken, eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE, fig.height = 6, fig.width = 12}
#remove all non-pg13 commenst when setting echo to true
library(tidyverse)
library(ggplot2)
library(maps)
...
econindicator <- econineq %>% filter(year == 2020)
desi <- desidata %>% filter(period == "desi_2020")
gdpPERCAPITA <- gdpPERCAPITA %>% filter(year == 2020)

econindicator$gdp <- as.numeric(econindicator$gdp)
...
speedscatter <- left_join(econindicator, speedyear, by = "region")  %>%
  left_join(. , gdpPERCAPITA, by = "region")
head(speedscatter)
#options(repr.plot.width = 10, repr.plot.height =3) #seemingly does nothing
speedscatter %>% 
        drop_na(values) %>%
  ggplot(data = speedscatter, mapping = aes(x = GDP.per.capita, y = as.numeric(values), colour = gini)) +
  geom_point() + geom_smooth(method="lm", col="black")

speedscatter %>% #probelm
        drop_na(values) %>%
  ggplot(data = speedscatter, mapping = aes(x = as.numeric(gini), y = as.numeric(values), colour = region_wb)) +
  geom_point()
...
pricescatter %>% 
        drop_na(values) %>%
  ggplot(data = pricescatter, mapping = aes(x = as.numeric(values), y = GDP.per.capita, colour = region_wb)) +
  geom_point() + geom_smooth(method="lm", col="black")
